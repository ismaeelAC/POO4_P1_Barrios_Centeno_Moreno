...
import poo4_p1_barrios_centeno_moreno.TipoServicio;
...
private String fdp12;
private String valorfinalapagar;
private void procesarViajes(String numserv) {
    ArrayList<String> lineaviajes = LeeFichero("viajes.txt");

    for (String lineaViaje : lineaviajes) {
        String[] arregv = lineaViaje.split(",");
        if (numserv.equals(arregv[0])) {
            numpasa = arregv[1]; 
        }
    }
}

private void procesarPagos(String numserv) {
    ArrayList<String> lineapagos = LeeFichero("pagos.txt");

    for (String lineaPago : lineapagos) {
        String[] arrepa = lineaPago.split(",");
        if (numserv.equals(arrepa[2])) {
            fdp12 = arrepa[3]; 
            valorfinalapagar = arrepa[6]; 
        }
    }
}
...
private void procesarEncomiendas(String numserv) {
    ArrayList<String> lineasEncomiendas = LeeFichero("encomiendas.txt");

    for (String lineaEncomienda : lineasEncomiendas) {
        String[] arregloEncomienda = lineaEncomienda.split(",");
        if (numserv.equals(arregloEncomienda[0])) {
            cantidadProductos = Integer.parseInt(arregloEncomienda[2]);
            pesoEncomienda = Double.parseDouble(arregloEncomienda[3]);
            tipoEncomienda = TipoEncomienda.valueOf(arregloEncomienda[1]);
        }
    }
}
...
public TipoUsuario verificarusuario(ArrayList<String> ar) {
    ...
    //System.out.println(c1.getVehiculo().getTipo());
    vehiculos.add(v1);
...
public void mostrarmenu(TipoUsuario type) {
        ...
        case C:
            ...
            do {
                ...
                if (opc == 1) {
                 ...
                 String tpst=String.valueOf(TipoServicio.T);
                 Sistema.EscribirArchivo("servicios.txt",ID+","+tpst+","+cd1[0]+","+cd1[1]+","+inception+","+llegada+","+date+","+hour); 
                 ...
                 System.out.println("Subotal a pagar:"+sbt);
                 ...
                 String tpse=String.valueOf(TipoServicio.E);
                 Sistema.EscribirArchivo("servicios.txt",ID+","+tpse+","+cd1[0]+","+cd1[1]+","+inception2+","+llegada2+","+date2+","+hour2); 
                 servicios.add(e1);
    ...
    if (users.get(i).getTipoUsuario().equals(TipoUsuario.R)) {
        Conductor conductor = (Conductor) users.get(i);
        
        if (conductor.getVehiculo().getTipo().equals(TipoVehiculo.A)) {
            ArrayList<String> lineaserv = LeeFichero("servicios.txt");

            for (String linea : lineaserv) {
                String[] arreg = linea.split(",");
                if (conductor.getNombre().equals(arreg[3])) {
                    
            String numserv = arreg[0];
            String tiposerv = arreg[1];
            String origen = arreg[4];
            String destino = arreg[5];
            String fecha = arreg[6];
            String hora = arreg[7];
            procesarViajes(numserv);
            procesarPagos(numserv);
            Double vlp=Double.valueOf(valorfinalapagar);
            Integer numpasaje=Integer.valueOf(numpasa);
            FormaDePago fp1=FormaDePago.valueOf(fdp12);
            Integer numsv=Integer.valueOf(numserv);
            int cont=1;
            Taxi t1x=new Taxi(numpasaje,numsv,conductor.getNombre(),origen,destino,fecha,hora,vlp,fp1);
        ...
        } else if (conductor.getVehiculo().getTipo().equals(TipoVehiculo.M)) {
            servicioEncontrado = true;
            ArrayList<String> lineaserv = LeeFichero("servicios.txt");
            ...
            for (String linea : lineaserv) {
                ...
                if (conductor.getNombre().equals(arreg[3])) {
                    ...
                    String fecha = arreg[6];
                    String hora = arreg[7];
                    // Obtener valores para crear Encomienda
                    
                    procesarPagos(numserv);
                    procesarEncomiendas(numserv);
                    Double vlp = Double.valueOf(valorfinalapagar);
                    FormaDePago fp1 = FormaDePago.valueOf(fdp12);
                    Integer numsv = Integer.valueOf(numserv);
                    Encomienda encomienda = new Encomienda(pesoEncomienda,tipoEncomienda,cantidadProductos,numsv,conductor.getNombre(),origen,destino,fecha,hora,vlp,fp1);
            ...
            if (!servicioEncontrado) {
                System.out.println("No tiene ningún servicio de Encomienda asignado");
            }
        }
    }
}
